...
public class MatchingQueryServiceImpl implements MatchingQueryService {
    ...
    public List<MatchingHistoryResult> recentlyMyMatchingList() {
        ...
        return this.history(me, 1);
    }
    @Override
    public List<MatchingHistoryResult> history(Long memberId, int since) {
        ...
        Map<Long, List<InfluenceProfileImage>> groupBy
                = influenceProfileImageJpaRepository.findByInfluence_IdIn(collect).stream().collect(Collectors.groupingBy(it -> it.getInfluence().getId()));
